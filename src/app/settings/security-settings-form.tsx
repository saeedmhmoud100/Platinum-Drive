  ...
  const fetchSettings = async () => {
    try {
      ...
      setSettings({
        ...
        requireReauth: data.settings?.requireReauth ?? false,
        loginAlerts: data.settings?.loginAlerts ?? true,
        defaultSharePermission: data.settings?.defaultSharePermission || "view",
        defaultLinkExpiry: data.settings?.defaultLinkExpiry ?? 7,
      })
    } catch (error) {
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="download">عرض وتنزيل</SelectItem>
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2 md:col-span-2">
            ...
            <Select
              value={(settings.defaultLinkExpiry ?? 7).toString()}
              onValueChange={(value: string) => setSettings({ ...settings, defaultLinkExpiry: parseInt(value) || null })}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="flex items-center justify-between gap-4 rounded-lg border p-3.5 bg-primary/5">
            <div className="space-y-0.5 text-right flex-1">
              <Label htmlFor="requireReauth" className="text-sm font-medium">إعادة المصادقة للعمليات الحساسة</Label>
          ...
          <div className="flex items-center justify-between gap-4 rounded-lg border p-3.5 bg-primary/5">
            <div className="space-y-0.5 text-right flex-1">
              <Label htmlFor="loginAlerts" className="text-sm font-medium">تنبيهات تسجيل الدخول</Label>
