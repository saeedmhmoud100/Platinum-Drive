...
export function FolderCard({
  ...
  if (viewMode === "list") {
    return (
      <div
        className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4 border rounded-lg hover:bg-accent transition-colors cursor-pointer group"
        dir="rtl"
        onClick={handleOpen}
      >
        {/* Folder Icon */}
        <div className="shrink-0">
          <Folder className="h-6 w-6 sm:h-8 sm:w-8 text-blue-500 group-hover:text-blue-600" />
        </div>
              ...
              {onToggleFavorite && (
                <DropdownMenuItem onClick={handleToggleFavorite}>
                  <Star
                    className={cn(
                      "h-4 w-4 ml-2",
                      folder.isFavorite && "fill-yellow-500 text-yellow-500"
                    )}
                  />
  ...
  return (
    <div
      className="group relative border rounded-lg p-3 sm:p-4 hover:shadow-md transition-all bg-card cursor-pointer"
      dir="rtl"
      onClick={handleOpen}
    >
      ...
      <div className="flex flex-col items-center gap-2 sm:gap-3">
        <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-lg bg-blue-50 dark:bg-blue-950/20 flex items-center justify-center group-hover:bg-blue-100 dark:group-hover:bg-blue-900/30 transition-colors">
      ...
      {/* Actions Menu */}
      <div
        className="sm:opacity-0 sm:group-hover:opacity-100 transition-opacity absolute top-2 left-2"
        onClick={(e) => e.stopPropagation()}
      >
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
